  ...
  const handleMinus = () => {
    ...
    if (inCart && cartItem) {
      if (cartItem.quantity > 1) {
        updateQuantity(cartItem.product._id, cartItem.quantity - 1);
      } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            {/* left side - Image Container (Badges ටික මේක ඇතුළට දැම්මා) */}
            <div className="relative flex-center p-8 md:p-12 min-h-[320px] md:min-h-[480px] bg-gray-50/50">
              {/* Badges Layout */}
              <div className="absolute top-5 left-5 flex flex-wrap gap-1.5 z-10">
                ...
                {product.discount > 0 && (
                  <span className="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold bg-red-500 text-white rounded-full shadow-sm">
              ...
              <img
                src={product.image}
                alt={product.name}
                className="max-h-[360px] w-auto object-contain relative z-0"
              />
            </div>

            {/* right side - Details */}
            <div className="p-6 md:p-10 flex flex-col justify-center">
              <span className="text-app-text-light text-xs font-medium tracking-wider mb-2 capitalize">
                {categoryLabel}
              </span>

              <h1 className="text-app-green text-2xl md:text-3xl font-semibold mb-3">
                {product.name}
              </h1>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center border border-app-border rounded-xl overflow-hidden bg-white">
                  ...
                  <span className="px-5 text-sm font-semibold text-center min-w-[40px]">
                    {displayQuantity}
                  </span>
